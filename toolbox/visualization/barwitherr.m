...
tempColor = linspecer(length(handles.bar(1).YData), 'sequential');

% tempColor = jet(length(handles.bar));
% for i = 1:length(handles.bar(1).YData)
%     set(handles.bar(1).YData(i),'FaceColor', tempColor(i)) % Use color code
% %     set(handles.bar(i),'FaceColor', 'white') % Use all balck bar
% end
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        set(hErrorbar(col), 'marker', 'none', 'CapSize', 12, 'LineWidth', 3.5)
    ...
    set(hErrorbar, 'marker', 'none', 'CapSize', 12, 'LineWidth', 3.5)
end
